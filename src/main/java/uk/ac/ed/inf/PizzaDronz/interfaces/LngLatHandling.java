...
public interface LngLatHandling {
     ...
     */
    default boolean isInCentralArea(LngLat point, Region centralArea) {
        if (centralArea == null){
            throw new IllegalArgumentException("the named region is null");
        }
        if (!centralArea.getName().equals(SystemConstants.CENTRAL_REGION_NAME)) {
            throw new IllegalArgumentException("the named region: " + centralArea.getName() + " is not valid - must be: " + SystemConstants.CENTRAL_REGION_NAME);
        }
     ...
     */
    boolean isInRegion(LngLat position, Region region);
